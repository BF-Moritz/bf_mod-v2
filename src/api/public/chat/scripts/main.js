							...
							if (data.params.settings.twitchAccounts) {
								data.params.settings.twitchAccounts.forEach((account, i) => {
									...
									option.addEventListener('click', () => {
										const elements = twitchChatInputSelectContent.getElementsByClassName('possible-twitch-accounts');
...
async function addMessages(params) {
	if (params.hasOwnProperty('messages')) {
		if (Array.isArray(params.messages)){
			if (twitchChatWrapper.childElementCount > 0 && params.messages[0].index < +(twitchChatWrapper.firstElementChild.id.split("-")[2])) {
...
async function addMessage(message) {
	...
	emotes.forEach(emote => {
		if (emote['emoteStart'] === lastPartEnd) {
			...
			messageParts.push({type: 'text', message: message.message.message.slice(lastPartEnd, emote['emoteStart'])});
			messageParts.push({type: 'emote', message: emote['emoteID']});
		}
	...
	let isOnBottom = twitchChatWrapper.scrollTop + $("#twitch-chat-wrapper").height() + 10 >= twitchChatWrapper.scrollHeight
	...
	newMessageHeadName.addEventListener('click', async (event) => {
		const id = newMessageHeadName.classList.toString().split(" ").filter(value => value.startsWith('id-'))[0].substring(3)
	...
	if (twitchChatWrapper.childElementCount > 0 && message.index < +(twitchChatWrapper.firstElementChild.id.split('-')[2])) {
...
function convertTimestampToString(timestamp) {
	...
	const date = `${("00" + ts.getDate()).slice(-2)}.${("00" + (+ts.getMonth() + 1)).slice(-2)}.${("0000" + ts.getFullYear()).slice(-4)}`
	const time = `${("00" + ts.getHours()).slice(-2)}:${("00" + ts.getMinutes()).slice(-2)}:${("00" + ts.getSeconds()).slice(-2)}`
	return `${date} - ${time}`
}
...
async function getYTData(videoID, apiKey) {
	return new Promise(resolve => {
		...
		fetch('https://www.googleapis.com/youtube/v3/videos?id=' + videoID
			+ '&key=' + apiKey
			+ '&fields=items(id,snippet(channelId,title,categoryId),statistics)&part=snippet,statistics', {}).then(res => {
			if (res.ok) {
				res.json().then(data => {
					resolve(data);
				});
