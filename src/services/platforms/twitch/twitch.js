...
export class Twitch {
	...
	async importCommands() {
		await readFiles(path.join(externalFilesPath, '/commands'), async (dir, file, params) => {
			let cmdName = file.substring(0, file.indexOf('.js'));
			try {
				let cmdModule = await import('file://' + path.join(dir, file));
				if (await checkCommands(cmdName, cmdModule) && cmdModule.default.clients.includes(params.twitch.name)) {
	...
	async importEvents() {
		...
		await readFiles(path.join(externalFilesPath, '/events'), async (dir, file, params) => {
			let eventName = file.substring(0, file.indexOf('.js'));
			try {
				let eventModule = await import ('file://' + path.join(dir, file));
				if (await checkEvents(eventName, eventModule) && eventModule.default.clients.includes(params.twitch.name)) {
	...
	async importActions() {
		...
		await readFiles(path.join(externalFilesPath, '/actions'), async (dir, file, params) => {
