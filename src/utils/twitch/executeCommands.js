...
export async function executeCommands(message, twitchID, channel) {
	...
	if (message.startsWith(generalTwitchConfig.commandPrefix)) {
		let cmdArgs = message.substring(message.indexOf(generalTwitchConfig.commandPrefix) + 1).split(new RegExp(/\s+/));
		...
		if (cooldown.name === cmdName) {
			...
			try {
				...
				if (+cooldown.time + +cooldownTime <= +time) {
					...
					services.bot.client.say(channel, `Der Command !${cmdName} kann erst wieder in ${minutes} Minuten und ${seconds} Sekunden genutzt werden.`);
					return false
				}
			} catch (err) {
				console.error(err);
